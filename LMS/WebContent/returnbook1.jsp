   ...
   ">
              <thead>
                <tr>
 				...
 				String date1=request.getParameter("returndate1");
 				java.util.Date dt1=dtformat.parse(date1); 
    	        ...
    	        String sql="select * from issuebook where lno="+Lno+" ";
             	...
             	while(rs.next())
             	{
             		...
             		Date returndate1=rs.getDate(5);
             		int fine=rs.getInt(6);
             		LocalDate rrr=returndate1.toLocalDate();
             		//Period diff = Period.between(rrr, date2);
             		long finesum;
             		long noofdays=rrr.until(date2,ChronoUnit.DAYS);
             		long totalfine =noofdays*fine;
             		if(totalfine>0)
             		{
             			finesum=totalfine;
             		}
              ...
              <tbody>
                <tr>
                  ...
                  <td><a href="ReturnBook?item=<%=Lno1%>&item2=<%=bookid1%>&item4=<%=finesum%>">Return</a>
                       <a href="Reissue?item=<%=Lno1%>&item2=<%=bookid1%>&item3=<%=date1%>&item4=<%=finesum%>">Reissue</a></td>
                 <%
                 }
                  %>
                </tr>
